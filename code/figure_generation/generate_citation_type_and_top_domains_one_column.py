...
OUTPUT_PATH = "../../figures/citation_type_and_top_domains_one_column.pdf"
...
def create_domain_type_plot(ax, df_domain_info):
    ...
    ax.set_xlabel("number of citations", fontsize=9, labelpad=2)
...
def create_top_domains_plots(ax2, ax3, df_domain_info):
    ...
    for ax, df_top, title in zip(
        ...
        ax.xaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f"{int(x):,d}"))
        ax.set_xlabel("number of citations", fontsize=9, labelpad=2)
        ax.set_title(title, fontsize=10, y=0.93)
...
def main():
    ...
    gs = GridSpec(
        ...
        1,
        figure=fig,
        height_ratios=[0.25, 0.375, 0.375],
    )

    # Create subplots
    ax1 = fig.add_subplot(gs[0, 0])  # Top panel: Domain type
    ax2 = fig.add_subplot(gs[1, 0])  # Middle panel: GPT-4o mini domains
    ax3 = fig.add_subplot(gs[2, 0])  # Bottom panel: GPT-4o domains
    ...
    # Add panel markers
    ax1.text(-0.4, 1.05, "(a)", transform=ax1.transAxes, fontsize=10)
    ax2.text(-0.4, 1.05, "(b)", transform=ax2.transAxes, fontsize=10)
    ax3.text(-0.4, 1.05, "(c)", transform=ax3.transAxes, fontsize=10)
